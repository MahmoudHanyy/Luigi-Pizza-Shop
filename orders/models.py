...
class Order(models.Model):
    ...
    def get_total_price(self):
        total = 0
        if self.pizza: total += self.get_pizza_price(self)
    ...
    def get_pizza_price(self):
        ...
        if len(self.pizza.toppings) == 1:
            if self.pizza.toppings.all().fist().name == 'Cheese':
                return float(self.pizza.price) * int(self.pizza_quantity)
        else:
            return toppings_choices[f"{self.pizza.get_style_display()}_{self.pizza.get_size_display()}"][len(self.pizza.toppings)] * float(self.pizza_quantity)
